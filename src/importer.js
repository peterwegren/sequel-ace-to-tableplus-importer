...
class Import {
  constructor(prompt) {
    this.sequelAceFavorites = `${homedir}/Downloads/SequelAceFavorites.plist`;
    ...
    if (prompt.confirmContinue) {
      ...
      this.buildConnections(
        plist.parse(fs.readFileSync(this.sequelAceFavorites, "utf8"))[
          "SPConnectionFavorites"
        ]
      );
      this.createGroups();
      this.mapConnections();
   ...
   */
  backupConnections() {
    [this.tablePlusConnections, this.tablePlusConnectionGroups].forEach(
      (file) => {
        let oldFile, newFile;
        try {
          oldFile = fs.readFileSync(file, "utf-8");
          newFile = fs.createWriteStream(
            `${file}.backup-${new Date().getTime()}`
          );
  ...
  /**
   * Loops through exising Sequel Ace connections recursively (in case there are folders)
   */
  ...
  createGroups() {
    [
      ...new Set(this.newConnections.map((connection) => connection.group)),
    ].forEach((group) => {
      if (group) {
        this.newGroups.push({
          ID: group,
  ...
  /**
   * Maps the Sequel Ace fields to TablePlus
   */
  mapConnections() {
    let map = this.newConnections.map((connection) => {
      if (!connection.connections.length) {
        return {
          ...
          DatabaseHost: connection.connections.sshHost
            ? "127.0.0.1"
